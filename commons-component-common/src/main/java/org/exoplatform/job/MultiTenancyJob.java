...
public abstract class MultiTenancyJob implements Job {
  ...
  public void execute(JobExecutionContext context) throws JobExecutionException {
    ...
    if (TENANT_MODE) {
      ...
      if (tenantsService == null)
        ...
        try {
          ...
          repositoryName = (String)GET_NAME_METHOD.invoke(o);
        } catch (Exception e) {
    ...
    callMultiTenancyJob(container, context, repositoryName);
  }

  private void callMultiTenancyJob(ExoContainer container, JobExecutionContext context, String repositoryName) {
    try {
      if (repositoryName == null) {
         RepositoryService repoService = (RepositoryService) container.getComponentInstanceOfType(RepositoryService.class);
         repositoryName = repoService.getCurrentRepository().getConfiguration().getName();
      }
