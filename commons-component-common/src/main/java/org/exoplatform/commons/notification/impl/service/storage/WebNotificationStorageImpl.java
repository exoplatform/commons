...
public class WebNotificationStorageImpl extends AbstractService implements WebNotificationStorage {
  ...
  public boolean remove(long seconds) {
    ...
    String dateNodeName = ISO9075.encode(converDateToNodeName(cal));
    try {
...
//      strQuery.append("fn:name()='").append(dateNodeName).append("'");
      ...
      LOG.error("Failed to remove all notifications and delay date: " + dateNodeName, e);
      return false;
    } finally {
      NotificationSessionManager.closeSessionProvider(created);
    }
